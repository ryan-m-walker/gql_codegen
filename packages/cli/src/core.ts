 ...
 */
export async function generate(options: CoreGenerateOptions): Promise<CoreGenerateResult> {
 ...
 */
async function generateViaBinary(options: CoreGenerateOptions): Promise<CoreGenerateResult> {
  ...
  try {
    ...
    const result = await new Promise<CoreGenerateResult>((resolve, reject) => {
      ...
      proc.on('close', (code) => {
        if (code === 0) {
          ...
          reject(new Error(`gql-codegen exited with code ${code}`));
        }
      });
